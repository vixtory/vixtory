...
'''
    ...
    def mergeConfig(starkSecurityConfigClass = null) {
        StarkConfigurationHolder.config = null
    }

    def doWithSpring = {
        // This sections sets up most of the filter beans we'll need.
        
        log.debug "Master config: ${ConfigurationHolder.config}"

        mergeConfig()

        def starkConfig = StarkConfigurationHolder.config.starksecurity
...
//    	def pathFilterMap = applicationContext.getBean('pathFilterInvocationDefinitionMap')
	...
	private setAuthorizationsByConfig(pathFilterMap) {
        ...
        if (StarkConfigurationHolder.config.starksecurity) {
            def auths = StarkConfigurationHolder.config.starksecurity.authorizations
            auths?.each { entry ->
    ...
    def doWithWebDescriptor = {webXml ->
        ...
        // The stark security filter needs to go after the Spring char encoding filter (if present) 
        // and before the sitemesh filter.  Borrowed logic from the JSecurity plugin...
